...
class Tracker:
    ...
    def update(self, det: Dict[str, np.ndarray], det_features: Optional[np.ndarray], det_feat_valid: Optional[np.ndarray]) -> tuple[List[Track], np.ndarray]:
        ...
        # --- дебаг в консоли (используется ли ReID) ---
        fr = det.get("frame_idx", None)
        if isinstance(fr, (int, np.integer)) and (int(fr) % 200 == 0):
            print(
                f"[DBG] frame={int(fr)} "
                f"reid_status={self.last_debug.get('reid_status')} "
                f"beta_eff={self.last_debug.get('beta_eff')} "
                f"det_hi={self.last_debug.get('det_hi')} det_lo={self.last_debug.get('det_lo')} "
                f"mH={self.last_debug.get('m_high')} mL={self.last_debug.get('m_low')} "
                f"react={self.last_debug.get('react')} spawn={self.last_debug.get('spawn')}"
            )
...
class PersonReID:
    """Экстрактор признаков OSNet через FeatureExtractor из torchreid (совместимо с разными сборками)."""

    def __init__(self, weights_path: str, *, device: str):
        # 1) Пытаемся импортировать FeatureExtractor из разных возможных мест
        FeatureExtractor = None
        try:
            # вариант для deep-person-reid (часто так)
            from torchreid.utils import FeatureExtractor as FE  # type: ignore
            FeatureExtractor = FE
        except Exception:
            try:
                # вариант для вашей текущей структуры (torchreid.reid.*)
                from torchreid.reid.utils import FeatureExtractor as FE  # type: ignore
                FeatureExtractor = FE
            except Exception:
                try:
        ...
        if FeatureExtractor is None:
            raise RuntimeError(
                "Не найден FeatureExtractor в установленном torchreid.\n"
                "Проверьте, что torchreid содержит FeatureExtractor (а не только reid.metrics и т.п.)."
            )
